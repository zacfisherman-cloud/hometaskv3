...
const STORES = ['Supermarket','Fruit and veg','Butcher','Seafood','Deli','Bakery'];
...
Rules: 6-10 ingredients each with numeric qty (or null, fractions as decimals) and unit one of g, kg, ml, l, cup, tbsp, tsp, clove, can, bunch, head, slice, piece, pack — or null. The item field is the food name only, never quantities or units inside it. Keep names short and appealing.

STEPS — this is the part that most often comes out too thin, so follow it closely:
Write 6-9 steps. Each step is a complete, actionable instruction a confident
home cook could follow with zero guessing — the same level of detail you'd
expect from a real recipe card or cookbook, never a one-line summary. Every
step that involves heat, time, or a size/temperature MUST state the actual
number: pan size ("in a large 30cm pan"), heat level ("over medium-high
heat"), exact time ("cook for 6-8 minutes, stirring occasionally"), exact
oven temperature in °C, and visual/texture doneness cues ("until golden and
the internal temperature reaches 75°C" / "until the sauce coats the back of
a spoon"). Break compound actions into separate steps rather than cramming
prep+cook+rest into one line. Name specific techniques (sear, deglaze,
simmer, fold, rest, reduce) instead of vague verbs like "cook" or "prepare".
Bad step (too vague, do not write like this): "Cook the chicken until done."
Good step (this level of detail): "Season the chicken thighs with salt and
pepper, then sear skin-side down in a large pan over medium-high heat for
6-7 minutes until deeply golden. Flip and cook a further 5 minutes until the
internal temperature reaches 75°C, then rest on a plate for 5 minutes before
slicing."`;

  const { obj } = await askForJSON(env, system, user, 3200);
